                    ...
                    <div class="card-body">
                        ...
                        <table class="table table-bordered">
                            <thead>
                                <tr>
                                    <th width="5%">No</th>
                                    <th width="5%">Nama Kelas</th>
                                    <th width="35%">Subjek Kelas</th>
                            ...
                            <tbody>
                                ...
                                $sql_u = "SELECT k.id_kelas, k.nama_kelas, k.subjek_kelas, jp.nama_jenjang 
          FROM kelas k 
          LEFT JOIN jenjang_pendidikan jp 
          ON k.id_jenjang = jp.id_jenjang_pendidikan";
                                if (isset($_GET["katakunci"])) {
                                    $katakunci_kelas = $_GET["katakunci"];
                                    $sql_u .= " where k.nama_kelas LIKE '%$katakunci_kelas%' OR k.subjek_kelas LIKE '%$katakunci_kelas%'";
                                }
                                $sql_u .= " ORDER BY nama_kelas limit $posisi, $batas ";
                                $query_u = mysqli_query($koneksi, $sql_u);
                                $no = 1;
                                while ($data_u = mysqli_fetch_assoc($query_u)) {
                                    $id_kelas = $data_u['id_kelas'];
                                    $nama_kelas = $data_u['nama_kelas'];
                                    $subjek_kelas = $data_u['subjek_kelas'];
                                    $nama_jenjang = $data_u['nama_jenjang'];

                                    ?>
                                    <tr>
                                        <td><?php echo $no; ?></td>
                                        <td><?php echo $nama_kelas; ?></td>
                                        <td><?php echo $subjek_kelas; ?></td>
                                        <td><?php echo $nama_jenjang; ?></td>
                                        <td align="center">
                                            <a href="kelas_edit.php?data=<?php echo
                                                $id_kelas ?>" class="btn btn-sm btn-warning"><i
                                                    class="fas fa-edit"></i> Edit</a>
                                            <a href="javascript:if(confirm('Anda yakin ingin menghapus data
                    <?php echo $nama_kelas; ?>?'))window.location.href =
                    'kelas.php?aksi=hapus&data=<?php echo
                        $id_kelas; ?>&notif=hapusberhasil'" class="btn btn-sm btn-danger"><i class="fas fa-trash"></i>
                                                Hapus</a>
                                        </td>
                                    </tr>
                                    <?php $no++;
                    ...
                    $sql_u .= " ORDER BY nama_kelas limit $posisi, $batas ";
                    $sql_jum = "SELECT k.id_kelas, k.nama_kelas, k.subjek_kelas, jp.nama_jenjang 
            FROM kelas k 
            LEFT JOIN jenjang_pendidikan jp 
            ON k.id_jenjang = jp.id_jenjang_pendidikan ";
                    if (isset($_GET["katakunci"])) {
