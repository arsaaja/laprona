              ...
              <div class="form-group row">
                ...
                <div class="col-sm-7">
                  <textarea class="form-control" name="deskripsi" id="deskripsi"
                    rows="12"><?php echo $deskripsi; ?></textarea>
                </div>
              </div>
              <div class="form-group row">
                ...
                <div class="col-sm-7">
                  <?php
                  $sql_s = "SELECT id_master_soft_skill,soft_skill FROM master_soft_skill ORDER BY id_master_soft_skill";
                  $query_s = mysqli_query($koneksi, $sql_s);
                  while ($data_s = mysqli_fetch_row($query_s)) {
                    $id_ss = $data_s[0];
                    $ss = $data_s[1];
                    ?>
                    <input type="checkbox" name="soft_skill[]" value="<?php echo $id_ss; ?>" <?php if (in_array($id_ss, $array_ss)) { ?>checked="checked" <?php } ?>> <?php echo $ss; ?></br>
                  <?php } ?>
                </div>
              </div>
              <div class="form-group row">
                ...
                <div class="col-sm-7">
                  <?php
                  $sql_h = "SELECT id_master_hard_skill,hard_skill FROM master_hard_skill ORDER BY id_master_hard_skill";
                  $query_h = mysqli_query($koneksi, $sql_h);
                  while ($data_h = mysqli_fetch_row($query_h)) {
                    $id_hs = $data_h[0];
                    $hs = $data_h[1];
                    ?>
                    <input type="checkbox" name="hard_skill[]" value="<?php echo $id_hs; ?>" <?php if (in_array($id_hs, $array_hs)) { ?>checked="checked" <?php } ?>> <?php echo $hs; ?></br>
                  <?php } ?>
                </div>
              </div>
