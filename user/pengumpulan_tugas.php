...
$query_pengumpulan = "SELECT pt.id_pengumpulan, pt.link_drive, pt.status, pt.waktu_submit
                      FROM pengumpulan_tugas pt
                      WHERE pt.id_tugas = $id_tugas AND pt.id_siswa = $id_siswa";

$result_pengumpulan = mysqli_query($koneksi, $query_pengumpulan);
$pengumpulan = mysqli_fetch_assoc($result_pengumpulan);
...
<body>
    ...
    <main>
        <section class="kelas-info">
            <h1>Kelas <?php echo htmlspecialchars($data['subjek_kelas'] ?? ''); ?></h1>
            <p><?php echo htmlspecialchars($data['judul_tugas'] ?? ''); ?></p>
            <div class="skor-diberikan">
                Skor yang diberikan: <strong><?php echo htmlspecialchars($data['skor_tugas'] ?? ''); ?></strong>
            </div>
        </section>
        ...
        <section class="detail-tugas">
            <h2>Detail Tugas</h2>
            <div class="card">
                <p><strong><i class="fas fa-file-alt"></i>
                        <?php echo htmlspecialchars($data['tugas'] ?? ''); ?></strong></p>
                <p><i class="fas fa-clock"></i> Deadline:
                    <?php echo isset($data['deadline_tugas']) ? date("d/m/Y H:i", strtotime($data['deadline_tugas'])) : ''; ?>
                </p>

                <?php if (!empty($pengumpulan)): ?>
                    <p><i class="fas fa-link"></i> Link Pengumpulan:
                        <?php
                        // Perbaikan di sini: Menggunakan echo untuk link pengumpulan dari $pengumpulan['link_drive']
                        echo '<a href="' . htmlspecialchars($pengumpulan['link_drive'] ?? '') . '" target="_blank">';
                        echo htmlspecialchars($pengumpulan['link_drive'] ?? '');
                        echo '</a>';
                        ?>
                    </p>
                    <p><i class="fas fa-calendar-check"></i> Status:
                        <strong><?php echo ucfirst($pengumpulan['status'] ?? ''); ?></strong>
                        (<?php echo isset($pengumpulan['waktu_submit']) ? date('d/m/Y H:i', strtotime($pengumpulan['waktu_submit'])) : ''; ?>)
                    </p>
                <?php else: ?>
                    <p><i class="fas fa-info-circle"></i>Link Pengumpulan Belum Tersedia.</p>
                <?php endif; ?>

                <p><i class="fas fa-star"></i> Skor Maksimum: <?php echo htmlspecialchars($data['skor_tugas'] ?? ''); ?>
                </p>
            </div>

            <form method="post">
                <p><i class="fas fa-clock"></i> Deadline:
                    <?php echo isset($data['deadline_tugas']) ? date("d/m/Y H:i", strtotime($data['deadline_tugas'])) : 'N/A'; ?>
                ...
                <div class="form-group">
                    ...
                    <input type="url" name="link_drive" id="link_drive" class="form-control"
                        value="<?php echo htmlspecialchars($pengumpulan['link_drive'] ?? ''); ?>" required
                ...
                <div class="actions">
                    <button type="submit" name="tandai_selesai" class="btn">
                        ...
                        <?php echo $pengumpulan ? 'Perbarui Pengumpulan' : 'Tandai Selesai'; ?>
                    </button>
                </div>
            </form>
        </section>
